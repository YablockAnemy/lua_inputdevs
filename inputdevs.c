...
int luaopen_inputdevs(lua_State *L) {
	...
	struct table {
		...
		{"EVENTS", event_types_decode},
		{"EV_SYN", synchronization_events_decode},
		{"EV_KEY", keys_and_buttons_decode},
		{"EV_MSC", misc_events_decode},
		{NULL, NULL}
	};
	struct tablesTable {
		char *name;
	...
	for(int i1 = 0; tables[i1].tables; i1++) {
		lua_pushstring(L, tables[i1].name);
		lua_newtable(L);
		for(int i2 = 0; tables[i1].tables[i2].elements; i2++) {
			lua_pushstring(L, tables[i1].tables[i2].name);
			lua_newtable(L);
			for(int i3 = 0; tables[i1].tables[i2].elements[i3].value; i3++) {
				lua_pushinteger(L, tables[i1].tables[i2].elements[i3].index);
				lua_pushstring(L, tables[i1].tables[i2].elements[i3].value);
				lua_settable(L, -3);
			}
			lua_settable(L, -3);
		}
